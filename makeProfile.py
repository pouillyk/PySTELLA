...
def takeBackResLine(outFile, wlLine, velMin, velMax, velStep, inFile):
    ...
    if inFile!=None:
        ...
        print('Please check radial velocity, vsini, veiling')
        print('Current values:', par.vrad[0], par.vsini, par.veil)
        plt.figure()
        plt.title(r'Please check normalisation, V$_r$, vsini, veiling')
        plt.plot(lp.vel, lp.flux[:,0], 'k', label='Observation')
        plt.plot(lp.vel, (f_t(lp.vel)+par.veil)/(1+par.veil), 'r', label='Template')
        plt.legend()
        plt.show()
        ...
        if ask=='n':
            ...
            exit()
        allFlux=lp.flux - (f_t(lp.vel)[:,None]+par.veil)/(1+par.veil)
        allDflux = lp.dflux + f_dt(lp.vel)[:,None]
        ...
        for i in range(par.nbobs):
            ...
            if i==0:
                ...
                plt.figure()
                plt.title(r'Please check V$_r$, vsini, veiling')
                plt.plot(velInterp, flux, 'k', label='Observation')
                plt.plot(velInterp, (f_t(velInterp)+par.veil)/(1+par.veil), 'r', label='Template')
                plt.legend()
                plt.show()
                ask = str(input('Everything OK (y/n)? '))
                if ask=='n':
            ...
            flux = flux - (f_t(velInterp)+par.veil)/(1+par.veil) 
